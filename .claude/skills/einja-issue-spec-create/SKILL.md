...
2. ヒアリング観点チェックリストで不明点を洗い出す:
   ...
   | 非機能要件 | パフォーマンス、セキュリティ、ユーザビリティ | レスポンス要件、認証要件、エラー状態・ローディング・フォーカス管理の方針 |
...
**Phase 1a**: requirements-generatorエージェントで requirements.md 生成
   ...
   - **追加指示（呼び出し時にプロンプトに含める）**:
     ...
     - `UX` カテゴリACには以下を含めること:
       ...
       - ローディング表示とその解除
       - エラーメッセージの位置とフォーカス移動
       - 4状態実装（disabled/error/empty/loading）
       - 操作後フィードバック（成功/失敗メッセージ）
...
- **追加指示（呼び出し時にプロンプトに含める）**:
  ...
  - 少なくとも `初期表示`、`保存フロー`、`バリデーション`、`権限制御`、`エラーハンドリング`、`ユーザビリティ（4状態・フォーカス・多重送信防止）` の観点をカバーすること。
  - **外部API連携がある場合の必須記載事項**:
