...
- [ ] 2.1 ユーザー作成機能
  - 2.1.1 ユーザー作成機能の実装（TDD）
    - Domain〜UIまでフルスタック実装
    - **要件**: Story 1
    - **完了条件**: ユーザー作成ができること（AC1.UI.N.001, AC1.VAL.E.001 等 Story 1 の主要AC を満たす）

- [ ] 2.2 ユーザー閲覧機能（一覧・詳細）
  - 2.2.1 ユーザー閲覧機能の実装（TDD）
    - Domain〜UIまでフルスタック実装
    - **要件**: Story 2, 3
    - **完了条件**: 一覧・詳細が表示できること（Story 2 および Story 3 の主要AC を満たす）

- [ ] 2.3 ユーザー更新・削除機能
  - 2.3.1 ユーザー更新・削除機能の実装（TDD）
    - Domain〜UIまでフルスタック実装
    - **要件**: Story 4, 5
    - **完了条件**: 編集・削除ができること（Story 4 および Story 5 の主要AC を満たす）
```
...
- [ ] 2.1 ユーザー作成機能
  ...
  - 2.1.1 ユーザー作成機能の実装（TDD）
    ...
    - **要件**: Story 1
    - **依存関係**: 1.1
    - **完了条件**: ユーザー作成ができ、全テストがパス（AC1.UI.N.001, AC1.VAL.E.001 等を満たす）
...
- [ ] 1.1 Server Core構築とDB設定
  ...
  **使用Skill**: [steering:api-development]
  ...
  - 1.1.1 packages/server-core初期化とtsconfig設定
    ...
    - 依存パッケージのインストール
    - **要件**: Story 1
    - **実装AC**: AC1.UI.N.001（このタスクで実装するAC番号）
    - **依存関係**: なし
    - **完了条件**: server-coreパッケージが初期化されること（AC1.UI.N.001を満たす）
  ...
  - 1.1.2 DB接続設定とマイグレーション
    ...
    - マイグレーションの実行
    - **要件**: Story 1
    - **実装AC**: AC1.VAL.E.001, AC1.NAV.N.001（このタスクで実装するAC番号）
    - **依存関係**: 1.1.1
    - **完了条件**: DBに接続できること（AC1.VAL.E.001〜AC1.NAV.N.001を満たす）
...
**メタデータ項目（全て必須）**:
- **要件**: 対応するStory番号
- **実装AC**: このタスクで実装するAC番号（例: `AC1.UI.N.001, AC1.VAL.E.001`）。該当なし（基盤構築等）の場合は `なし（理由）` を明記
...
- **タスクレベル**: タスクのメタデータとして他の必須項目と同列（4スペースインデント）に記載。タスクグループレベルの指定をオーバーライドする。**サブエージェントは1つのみ指定可能**
