...
3. 全story完了後に以下を生成（qa-generator本体が実施）:
   - `{指定ディレクトリ}/qa-test.md`（複数ストーリーをまたぐシナリオテスト仕様。spec直下。旧称: scenarios.md）
   - `{指定ディレクトリ}/qa-tests/README.md`（QAテストガイド）
...
## 出力
- **qa-test.md**: シナリオテスト仕様（複数タスクをまたぐ継続操作フロー）※旧称 `scenarios.md` から統一。**`{指定ディレクトリ}/qa-test.md`（spec直下）として保存**
- `{指定ディレクトリ}/qa-tests/` ディレクトリとして以下を保存:
...
```
{spec_dir}/
├── qa-test.md          # シナリオテスト仕様（spec直下）（必須）※旧称 scenarios.md
└── qa-tests/
...
6. **シナリオテスト設計（必須）**:
   ※ 並列生成モードの場合、qa-test.mdは全story{N}.md完了後に生成すること。各storyのAC番号を正しく参照するため。
   - requirements.mdの受け入れ条件を分析し、複数タスクをまたぐフローを特定
   - シナリオは「画面操作や契約確認のまとまり」で切る（Story単位ではない）
   - 各シナリオに「関連Story」と「関連AC」（新AC体系）の両方をマッピング
   - AC実装完了ごとの実施可能範囲を明記
   - qa-test.mdとして **spec直下（`{指定ディレクトリ}/qa-test.md`）** に出力（旧称: scenarios.md から統一。`qa-tests/` 配下ではなく spec直下）
...
## qa-test.mdの構造（旧称: scenarios.md）

qa-test.mdファイルは、この構造に従います：
...
- [ ] **qa-test.md（旧称: scenarios.md）が spec直下（`{指定ディレクトリ}/qa-test.md`）に作成され、複数タスクをまたぐフローが定義されている**
