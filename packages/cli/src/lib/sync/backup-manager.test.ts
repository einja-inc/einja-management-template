...
describe("BackupManager", () => {
  ...
  describe("backupFiles", () => {
    it("複数のファイルをバックアップする", async () => {
      ...
      // バックアップファイルの内容を確認
      const backup1Content = await fs.readFile(backupMap.get(files[0]) as string, "utf-8");
      const backup2Content = await fs.readFile(backupMap.get(files[1]) as string, "utf-8");
  ...
  describe("restoreFile", () => {
    ...
    it("バックアップファイルが存在しない場合、エラーをスローする", async () => {
      await expect(manager.restoreFile("/nonexistent/backup.txt", "test.txt")).rejects.toThrow(
        "バックアップファイルが存在しません"
      );
    });
  });
