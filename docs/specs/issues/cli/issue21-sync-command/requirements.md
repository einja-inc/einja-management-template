...
- **期待効果**: 開発者の手動マージ時間を80%削減、テンプレート改善の反映率100%達成
...
| アクター | 説明 |
|---------|------|
| 開発者 | einja-inc orgメンバー。CLIを直接実行してテンプレート同期を行う |
...
| UC | ユースケース | アクター | 説明 |
|----|-------------|---------|------|
| UC1 | 基本同期を実行 | 開発者 | テンプレート更新をプロジェクトにマージ |
| UC2 | 差分を事前確認 | 開発者 | `--dry-run`で変更内容を確認（ファイル変更なし） |
| UC3 | 選択的同期 | 開発者 | `--only agents,commands`で特定カテゴリのみ同期 |
| UC4 | 強制上書き | 開発者 | `--force`でローカル変更を無視して上書き |
