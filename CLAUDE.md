...
1. **該当Skillの選定・読み込み** — Read ツールで直接実施
   - 「サブエージェント委託ルール」のSkill表・サブエージェント表を参照し、今回の作業に該当するSkillを特定する
...
5. **実装で使用するSkill・サブエージェントの選定** — 親プロセスが直接実施
   - 「サブエージェント委託ルール」のSkill表・サブエージェント表の両方と照合し、各作業にSkill（Skill tool）かサブエージェント（Agent tool）かを割り当てる
