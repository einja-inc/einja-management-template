...
graph TB
    ...
    subgraph "cli-template:update スクリプト<br/>（内部開発用）"
        UC10[プリセット更新を実行]
...
| UC10 | プリセット更新を実行 | CLI開発者 | `pnpm cli-template:update`でプロジェクトのコンテンツをCLIプリセットに反映（内部スクリプト） |
...
**As a** CLIパッケージ開発者
**I want to** `pnpm cli-template:update`でプロジェクトの最新コンテンツをCLIプリセットに反映したい
...
```
プロジェクトの .claude/
    ↓ (pnpm cli-template:update) ← 本ストーリーで実装
...
- 実行方法: `pnpm cli-template:update [options]`
...
- [ ] **AC10.2**: Given: `--preset minimal`オプション指定
             When: `pnpm cli-template:update --preset minimal`を実行
...
- [ ] **AC11.1**: Given: `--json`オプション指定
             When: `pnpm cli-template:update --json`を実行
...
### プリセット更新のディレクトリ構造要件
#### cli-template:updateスクリプトのディレクトリマッピング
**要件内容**:
- プロジェクトからCLIプリセットへの逆方向コピー（内部開発用スクリプト `pnpm cli-template:update`）
- ディレクトリマッピング:
