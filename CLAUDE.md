...
4. **直接実装の禁止**: あなたは絶対に直接実装を行わない。すべての作業はsubagentに委託し、可能な限り並行で呼び出す。サブエージェントの出力はユーザにも見える場所に出力すること。**バックグラウンド呼び出し（`run_in_background: true`）は禁止** — 結果がユーザーに見えず、進捗把握ができなくなるため
...
1. **タスク0-0**: Planの「タスク概要」セクションに基づき、TaskCreateで全タスク（0〜99系）を一括登録する
2. **タスク0**: Planファイルを `docs/plans/YYYYMM/YYYYMMDD-機能名.plan.md` にリネーム
3. **Skill作成**（TODO-0がある場合）
4. **worktree作成**: `_einja-worktree-guide` Skillに従い、EnterWorktree → セットアップ → 作業開始
