...
1.5. **UIデザインファイルの確認**: specディレクトリに`ui-design.pen`が存在するか確認 → パスを記録
2. **存在チェック**:
...
5. **対応UIデザインの記録**: 各タスクの`**対応UIデザイン**: ui-design.pen「フレーム名」`からフレーム名を記録
...
while (未完了タスクが存在):
  ...
  4. Task ツールで複数の task-executer を並列起動:
     - 各 task-executer のpromptに以下を含める（ハイブリッド方式）:
       ...
       h. UIデザイン参照（対応UIデザインメタデータが存在する場合）→ 下記「UIデザイン参照プロンプト」セクションを埋め込む
...
#### UIデザイン参照プロンプト（対応UIデザインが存在する場合に埋め込む）

**自動ルーティングルール**: 以下の条件をすべて満たす場合、`task-executer`ではなく`design-engineer`サブエージェントに自動委託する:
1. `対応UIデザイン`メタデータが存在する
2. `実行サブエージェント`が未指定（明示的に別のサブエージェントが指定されている場合はそちらを優先）
3. タスク名にUI実装を示すキーワードが含まれる（コンポーネント、画面、レイアウト、スタイル、UI、フォーム、表示、ビュー等）

**除外条件**: タスク名がバックエンド中心のキーワードのみで構成される場合（API、DB、バッチ、マイグレーション、シード、バリデーション、Repository、UseCase等）はルーティングしない。`対応UIデザイン`はタスクグループから継承される場合があるため、バックエンド専用タスクに誤ってルーティングしないよう注意。

**workerプロンプトに追加するセクション**:
```
## UIデザイン参照
