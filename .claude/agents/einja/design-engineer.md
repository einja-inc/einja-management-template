...
## UIデザイン忠実性の自己検証（ui-design.pen参照時の必須ステップ）

`ui-design.pen`を参照して実装した場合、**実装完了報告の前に**以下の検証を必ず実施すること。

### 検証プロセス

1. **デザインノード構造の取得**: Pencil MCP `batch_get`で`ui-design.pen`の対応フレームのノード構造を取得する
2. **主要プロパティの照合**: 実装コードの主要値とPencilノードのプロパティを照合する
   - レイアウト方向（flex-direction, display）
   - カラー値（background, color, border-color）
   - サイズ（width, height, font-size）
   - opacity
   - gap / padding / margin
   - border-radius
3. **乖離の修正**: 乖離がある場合は修正してから完了報告する
4. **デザイン要点サマリの作成**: Pencilから抽出した主要プロパティ一覧をテキストで作成する

### デザイン要点サマリの出力形式

完了報告に以下の「デザイン要点サマリ」セクションを必ず含めること。このサマリはtask-reviewerが後続のデザイン忠実性チェックで使用する。
...
### Pencil MCP未起動時のフォールバック

Pencil MCPが利用できない場合は、design.mdのテキスト仕様に基づいて実装し、デザイン要点サマリには「Pencil MCP未起動のためテキスト仕様ベースで実装」と記載する。
