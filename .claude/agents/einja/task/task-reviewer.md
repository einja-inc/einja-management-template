...
AskUserQuestion:
  ...
  options:
    ...
    - label: "設計仕様を更新"
      description: "メリット: 実装の良い点を活かせる。デメリット: 設計変更の調整コスト、他の関連仕様との整合性確認が必要"
    - label: "ロールバックして再実装"
      description: "メリット: クリーンな状態から再スタート。デメリット: スケジュール遅延、これまでの作業が無駄になる"
```
...
AskUserQuestion:
  ...
  options:
    ...
    - label: "修正容易度順"
      description: "メリット: 早期に進捗を出せる。デメリット: 重大な問題が後回しになりリスクが残る"
    - label: "提案されたすべてを並行対応"
      description: "メリット: 網羅的に対応できる。デメリット: 時間がかかり、スケジュールへの影響が大きい"
```
