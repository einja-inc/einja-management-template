...
本フローの FN-005 は『打刻フロー』とも共有される機能（N対N関係の実証）。
打刻フローでは未打刻リマインド通知、承認フローでは承認依頼通知・結果通知の配信に使われる共通基盤機能。
-->
...
| FN-005 | 通知配信機能 | システムからのメール・アプリ内push通知を一元配信する（複数フロー共有） | - | §2.2 ステップ3, 7, 8 | MUST | **本機能は『打刻フロー』とも共有**（N対N関係）。承認依頼 / 結果通知 / 催促 / エスカレーション 等の配信を担う共通基盤 |
