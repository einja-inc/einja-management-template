...
3. **分割時の構成**:
   ...
   - **`design/implementation.md`**
     - 含まれるセクション: コンポーネントとインターフェース（データベース設計、API エンドポイント、フロントエンドコンポーネント）、実装上の注意点

   - **`design/quality.md`**
     - 含まれるセクション: エラーハンドリング、セキュリティ考慮事項、パフォーマンス最適化、テスト設計、マイグレーション戦略、モニタリングと分析
...
5. **分割実装の手順**:
   ```markdown
   # design/ディレクトリを作成（mkdir -p design/）
   # H2見出し（## ）およびH3見出し（### ）を目印にセクションを識別
   # 各パートに含めるセクションを抽出
   # Writeツールでdesign/配下に各ファイルを保存
   # design/README.mdを生成
   ```
...
`/docs/example/specs/tasks/20251101-example-task/design.md`
...
- 必ずサンプル設計書（/docs/example/specs/tasks/20251101-example-task/design.md）を参照して形式を統一