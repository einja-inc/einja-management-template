...
options:
  - label: "画面フロー全体をテスト（推奨）"
    description: "推奨理由: 修正の影響範囲を網羅的に確認。メリット: 想定外の副作用や回帰を早期発見。デメリット: テスト時間がやや長くなる"
  - label: "特定機能のみテスト"
    description: "修正箇所が限定的で影響範囲が明確な場合。メリット: テスト時間を短縮。デメリット: 想定外の副作用や回帰を見逃す可能性"
```
...
options:
  - label: "関連エンドポイント全体（推奨）"
    description: "推奨理由: 依存関係のある機能も含めて確認。メリット: データ連携や認証の問題を早期発見。デメリット: テスト時間がやや長くなる"
  - label: "単一エンドポイントのみ"
...
options:
  - label: "境界値テスト"
    description: "入力値の上限・下限を確認。メリット: エッジケースを網羅し、予期しない挙動を発見。デメリット: テストケース数が増加し時間がかかる"
  - label: "異常系テスト"
    description: "エラーハンドリングを確認。メリット: 本番環境でのエラー対処能力を検証。デメリット: 正常系テストより準備に時間がかかる"
  - label: "並行処理テスト"
    description: "複数リクエストの同時実行を確認。メリット: 競合状態やデータ不整合を検出。デメリット: 再現性の確保が難しく、環境構築が複雑になる"
```
