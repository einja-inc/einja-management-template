...
- **最終受け入れの人手E2Eシナリオ（種別 `人手E2E`）**: 最終受け入れがマージ / デプロイ後にしか確認できない場合、種別 `人手E2E` のシナリオ（人間が本番相当環境で業務フローを端から端まで操作し `E2E-ready` を確認する手順）をシナリオ一覧に1件以上含める。`deferred-to` 申し送りと対応させる。判定基準・記法の詳細は `docs/einja/steering/acceptance-criteria-and-qa-guide.md`「最終受け入れの readiness 下限」節を参照。
...
> **設計意図**: Phase / タスク番号へのシナリオ割当は後工程の tasks-generator が行う（qa-generator は tasks-generator より先に並列実行されPhaseやTask番号を持たない）。そのため scenarios.md の実施タイミングは **AC実装ベース**（「どのAC実装後に実行可能か」）でのみ記載し、Phase/タスク番号は書かない。
...
<!-- タスク番号への割当は後工程 tasks-generator が行うため、qa-generator 生成時点の scenarios.md には「関連タスク」（タスク番号）を記載しない。AC番号・Story番号のみで表現する。 -->

### 実施タイミング
- **AC2.NAV.N.001 実装後**: Step 1-3まで（部分実行：招待機能まで）
- **AC4.UI.N.001 実装後**: Step 1-5まで（部分実行：パスワード設定まで）
- **AC8.ERR.E.001 実装後**: 全Step（フル実行：認証機能実装で完了）
- **全AC実装後**: 全Step（リグレッション：CRUD変更の影響確認）
