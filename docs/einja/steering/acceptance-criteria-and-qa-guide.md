...
> **AC ID形式**: `AC-XXX-CAT-N-001`（XXX=機能略称、CAT=カテゴリ、N=正常系/E=異常系、001=連番）。カテゴリは「ACカテゴリの推奨定義」セクションを参照。

```
AC-XXX-CAT-N-001: <振る舞いの1文要約>
- 前提: <状態/入力/役割>
...
### 関連
- **受け入れ条件**: AC-XXX-UI-N-001, AC-XXX-NAV-N-001, AC-XXX-ERR-E-001（シナリオに関連するAC ID）

### 実施タイミング
- **AC-XXX-UI-N-001〜AC-XXX-UI-N-003実装後**: Step 1-3まで実施可能
- **AC-XXX-NAV-N-001〜AC-XXX-NAV-N-002実装後**: Step 1-5まで実施可能
- **AC-XXX-ERR-E-001実装後**: 全Step実施（フルシナリオ）
...
- 良い例:
  ```
  AC-USR-VAL-E-001: UserRepository はメールアドレス重複を防ぐ
...
- 良い例:
  ```
  AC-PWR-UI-N-001: PasswordResetUseCase はトークン発行後にメール送信イベントを発火
...
- 良い例:
  ```
  AC-USR-PERM-E-001: GET /users/:id は権限エラーを 403 で返す
...
- 良い例:
  ```
  AC-CLN-UI-N-001: cleanup ジョブは 30 日以上未使用のセッションを削除
...
- 良い例:
  ```
  AC-LGN-ERR-E-001: ログインフォームは誤ったパスワードで API エラーを表示
