...
**完了**（チェックボックスをON、着手中マークを削除）:
```markdown
- [x] **1.1 タスクグループ名**
```

**重要**: 着手中マークはHTMLコメント形式（`<!-- 🔄 着手中 -->`）を使用します。これにより、チェックボックスの状態を保持しつつ、視覚的に着手中であることを示せます。

### Issueのライフサイクル

1. **Issue作成**: `/spec-create`コマンドで仕様書作成時に自動生成
2. **タスクグループ選定**: `/task-exec #{issue_number}`でタスクグループを選定・着手中マークを付与
3. **タスクグループ実行**: 実装・テスト・レビューを実施
4. **タスクグループ完了**: チェックボックスをON、着手中マークを削除
5. **Issue完了**: すべてのタスクグループが完了したらIssueをClose
