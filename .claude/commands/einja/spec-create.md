...
options:
  - label: "はい、TDDで進める（推奨）"
    description: "推奨理由: ビジネスロジック、データ変換、外部API連携では品質向上に効果的。詳細は docs/einja/steering/development/testing-strategy.md を参照"
  - label: "実装後にテストを追加"
    description: "シンプルなUI変更など、TDDが過剰な場合に選択"
  - label: "テスト最小限"
    description: "プロトタイプや実験的実装の場合に選択"
```
...
AskUserQuestionツールで以下を確認：
```
question: "タスクの要件は明確ですか？以下の観点で確認してください"
header: "📋 要件確認"
description: |
  - 正常系: 一番よくある使われ方は明確か？
...
options:
  - label: "明確（仕様書作成を進める）"
    description: "正常系、異常系、境界値が明確に定義されている"
  - label: "不明確（追加質問で明確化）"
    description: "上記の観点で不明な点があるため、追加質問で要件を明確化する"
```
