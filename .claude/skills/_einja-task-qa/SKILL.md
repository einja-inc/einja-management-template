...
#### 環境前提条件チェック（動作確認前に必須実施）

動作確認の実施前に、以下を確認する:

1. **環境変数の特定**: 実装対象コード内の `process.env.` 参照を検索（Grepツール使用）し、外部サービス用の環境変数を特定する
2. **設定状況の確認**: 特定された環境変数が `.env` / `.env.local` に設定されているか確認する
3. **未設定時の判定**:
   - 未設定の環境変数がある場合 → **FAILURE（failureCategory=D: 環境問題）** 判定
