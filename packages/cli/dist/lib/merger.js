...
import { getCorePath, getPresetPath, getScaffoldsPath, getTemplatesPath, processTemplateFile, } from "./file-system.js";
/**
 * コアsettings.jsonとプリセット設定をマージ
 */
export function mergeSettings(core, preset) {
    return Object.assign(Object.assign({}, core), { permissions: {
 ...
 * .claudeディレクトリを生成
 */
export async function generateClaudeDirectory(targetPath, presetConfig) {
    const corePath = getCorePath();
    const presetPath = getPresetPath(presetConfig.name);
    // 1. コアファイルをコピー
    await copyAndProcessDirectory(path.join(corePath, "agents"), path.join(targetPath, "agents"), presetConfig.variables);
    await copyAndProcessDirectory(path.join(corePath, "commands"), path.join(targetPath, "commands"), presetConfig.variables);
    // 2. コアsettings.jsonを読み込み・マージ
    const coreSettingsPath = path.join(corePath, "settings.json");
    if (await fs.pathExists(coreSettingsPath)) {
        const coreSettings = await fs.readJson(coreSettingsPath);
    ...
    if (await fs.pathExists(presetAgentsPath)) {
        await copyAndProcessDirectory(presetAgentsPath, path.join(targetPath, "agents"), presetConfig.variables);
    }
    const presetCommandsPath = path.join(presetPath, "commands");
    if (await fs.pathExists(presetCommandsPath)) {
        await copyAndProcessDirectory(presetCommandsPath, path.join(targetPath, "commands"), presetConfig.variables);
    }
}
/**
 * ディレクトリをコピーしてテンプレート変数を展開
 */
async function copyAndProcessDirectory(srcDir, destDir, variables) {
 ...
 * ドキュメントテンプレートをコピー
 */
export async function copyDocTemplates(targetPath) {
 ...
 * CLAUDE.mdを生成
 */
export async function generateClaudeMd(targetPath, variables) {
 ...
 * ステアリングドキュメントをコピー
 */
export async function copySteeringDocs(targetPath) {
    const scaffoldsPath = getScaffoldsPath();
