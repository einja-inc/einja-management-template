...
describe("MetadataManager", () => {
  ...
  describe("load", () => {
    ...
    it("不正なJSON形式の場合、エラーをスローする", async () => {
      await fs.writeFile(path.join(tempDir, ".einja-sync.json"), "invalid json", "utf-8");
  ...
  describe("save", () => {
    it("メタデータをJSON形式で保存する", async () => {
      ...
      const content = await fs.readFile(path.join(tempDir, ".einja-sync.json"), "utf-8");
  ...
  describe("getBaseContent", () => {
    it("ファイルの内容とハッシュを取得する", async () => {
      ...
      const expectedHash = createHash("sha256").update(testContent, "utf8").digest("hex");
    ...
    it("ファイルが存在しない場合、エラーをスローする", async () => {
      ...
      await expect(manager.getBaseContent(nonExistentFile)).rejects.toThrow("ファイル");
    });
  });
  ...
  describe("updateFileHash", () => {
    ...
    it("既存のファイルハッシュを上書きする", async () => {
      ...
      const updated = await manager.updateFileHash(metadata, filePath, newContent);
