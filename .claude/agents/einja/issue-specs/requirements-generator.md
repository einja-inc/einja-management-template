...
2. **不明点の解消プロセス**
   ...
   - **手段1: 既存コード・ドキュメントの調査**
     ...
     - **【必須】過去Plan・類似Issue・既存実装の検索**
       ...
       - 調査結果は「実装参考情報」セクションの参考リソースに記載する
...
| 対象領域 | 推奨 | 理由 |
|---------|------|------|
| [対象領域1] | [Skill名またはサブエージェント名] | [推奨理由] |
| [対象領域2] | [Skill名またはサブエージェント名] | [推奨理由] |

### 参考リソース
- 類似Issue: [該当するIssueがあれば記載]
- 類似Plan: [docs/plans/配下の類似Planがあれば記載]
- 参考steering: [関連するsteering文書があれば記載]
- 既存実装: [参考になる既存実装があれば記載]
```

**「実装参考情報」セクションについて:**
- このセクションは要件分析の過程で調査した情報を基に生成する。上記テンプレートは構造の例であり、実際の内容は要件の特性に応じて適切な推奨Skill/サブエージェントと参考リソースを記載すること
- 推奨Skill/サブエージェント: CLAUDE.mdの「委託ルール」に記載されたサブエージェント・Skillから、要件の特性に合ったものを選定する
- 参考リソース: ステップ0で実施する過去Plan/Issue検索・既存実装調査の結果をまとめる
- このセクションはtasks-generatorが参照してタスクへのSkill/サブエージェント割り当てに活用する
...
- [ ] すべてのリスクに軽減戦略がある
- [ ] 実装参考情報に推奨Skill/サブエージェントと参考リソースが記載されている
...
4. **要件定義書作成（サンプルと同じ順序で）**:
   ...
   - **成功指標**: 測定可能な目標
   - **タイムライン**: フェーズ分けした実装計画
   - **実装参考情報**: 推奨Skill/サブエージェント、参考リソース（類似Issue・Plan・steering・既存実装）
