...
`対応UIデザイン`メタデータを付与するタスクには、以下のルールで`実行サブエージェント`を自動設定する:

- `対応UIデザイン`が付与されたタスク → `実行サブエージェント: [design-engineer]` を自動設定
- ただし、明示的に別の`実行サブエージェント`が指定されている場合はそちらを優先
- タスクグループレベルで`対応UIデザイン`を指定した場合、配下の全UIタスクに`[design-engineer]`が継承される
- **例外**: バックエンド専用タスク（API、DB、バッチ等）にはタスクレベルで異なるサブエージェントを個別指定すること
