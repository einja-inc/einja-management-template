...
凡例: `[親]` = 親プロセスが直接実行、`[Skill]` = Skill tool呼び出し、`[サブエージェント]` = Agent tool委託

1. `[親]` 計画策定に必要なSkillを選定・読み込む
   - 「サブエージェント委託ルール」の対応表を参照し、今回の作業に該当するSkillを特定する
   - 該当SkillのSKILL.mdを `Read` で読み込み、ルール・制約を把握する
   - 把握した内容をplanファイルの具体指示に反映する
2. `[サブエージェント]` 問題・要件を調査・分析する（`Explore` サブエージェント）
3. `[親]` `docs/plans/` の過去Planを検索し、類似対応がないか確認する（`Grep`/`Glob` で直接実施）
   - 類似Planがあれば知見を計画に取り入れる
   - 類似Planの存在はステップ4でSkill化の強い根拠とする
4. `[Skill]` `einja-skill-first` で「Skill を先に作るべきか」を評価する
...
5. `[親]` 実装で使用するSkill・サブエージェントを選定する
   - 「サブエージェント委託ルール」と照合し、各作業に適切なSkill/サブエージェントを割り当てる
   - 選定結果はplanファイルの「使用予定Skill・サブエージェント」セクションに記載する
6. `[親]` planファイルに計画を記述する
7. `[親]` ExitPlanMode で承認を得る
