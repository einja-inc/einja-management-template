...
各サブエージェント（task-executer、task-reviewer、task-qa、task-committer、task-modification-analyzer）の呼び出し後、
...
**選択肢**:
- **小規模（ロジック修正のみ）**
  - 説明: 単一ファイルの軽微な修正。task-executerのみで対応
- **中規模（複数ファイル修正）**
  - 説明: 関連ファイルも含めた修正。task-executer → task-reviewer
- **大規模（設計変更が必要）**
  - 説明: requirements.md/design.mdの修正後に全フローを再実行
