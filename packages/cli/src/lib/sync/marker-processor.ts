...
export class MarkerProcessor {
   ...
   */
  private static readonly MARKDOWN_START_PATTERN = /^<!--\s*@einja:managed:start\s*-->$/;
  private static readonly MARKDOWN_END_PATTERN = /^<!--\s*@einja:managed:end\s*-->$/;
  ...
  private isEndMarker(line: string): boolean {
    return (
      MarkerProcessor.MARKDOWN_END_PATTERN.test(line) || MarkerProcessor.YAML_END_PATTERN.test(line)
    );
  }
}
