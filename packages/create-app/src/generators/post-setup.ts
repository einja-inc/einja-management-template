...
export async function execPostSetup(
  ...
  // 4. @einja-inc/dev-cli init
  if (config.setupEinjaCli) {
    const einjaSpinner = ora("@einja-inc/dev-cli を初期化中...").start();
    try {
      await execa("npx", ["--yes", "@einja-inc/dev-cli@latest", "init", "--force", "--no-backup"], { cwd: targetPath });
      einjaSpinner.succeed("@einja-inc/dev-cli を初期化しました");
    } catch (error) {
      einjaSpinner.fail("@einja-inc/dev-cli の初期化に失敗しました");
      logger.warn("後で手動で 'npx --yes @einja-inc/dev-cli@latest init' を実行してください");
    }
  }
