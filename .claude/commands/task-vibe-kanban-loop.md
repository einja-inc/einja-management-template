---
description: "タスクファイルから実行可能なタスクを自動選定し、Vibe-Kanbanに登録して実行を継続するループコマンド。自然言語でタスクファイルパス、最大タスク番号、ベースブランチを指定できます。"
allowed-tools: Task, Read, Write, Edit, Bash, Grep, Glob, mcp__serena__*, mcp__vibe_kanban__*, AskUserQuestion
---

# タスク自動実行ループ

このコマンドは、指定されたタスクファイルから実行可能なタスクを自動的に選定し、Vibe-Kanbanに登録して実行を継続するループを実行します。
...
1. **タスクファイルパス**（必須）
   - 例: 「monorepoのタスクファイルで実行して」
   - 例: 「docs/specs/tasks/monorepo/20251104-monorepo-turborepo-nextjs-setup/tasks.md」

2. **最大タスク番号**（オプション）
   - **Phase番号で指定**: 「Phase 4まで」→ 4 または 4.X
   - **タスクグループ番号で指定**: 「タスクグループ4.2まで」→ 4.2
   - **タスク番号で指定**: 「タスク4.1.2まで」→ 4.1.2
   - 例: 「全部実行して」→ all
...
引数で指定されていない場合、以下の選択肢を提示：

```markdown
**質問**: タスクファイルのどのフェーズまでのタスクを実行しますか？

**選択肢**:
- すべてのタスクを実行（推奨）
- Phase 4まで（例: 4 または 4.X）
- タスクグループ4.2まで（例: 4.2）
- タスク4.1.2まで（例: 4.1.2）
- 特定の番号まで（Phase/タスクグループ/タスク番号を入力）
```

### 2. 実行フェーズの確認（オプション）

**ユーザーから指定がある場合**: このステップをスキップ

1. **タスクファイルを読み込んでフェーズ状況を確認**
   - どのフェーズまで完了しているかをチェック
   - 未完了のフェーズを特定
...
タスクファイルパスからブランチ名を生成し、作成元を確認：

1. **タスクファイルパスを解析**
   - パス例: `docs/specs/tasks/monorepo/20251104-turborepo-setup/tasks.md`
   - ディレクトリ: `monorepo`
   - タスク名: `20251104-turborepo-setup`
   - タスクブランチ名: `feat/monorepo/20251104-turborepo-setup`

2. **タスクブランチの存在確認**
   - 既に存在する場合 → そのまま使用
   - 存在しない場合 → 作成元を確認

3. **タスクブランチ作成元の確認**（ブランチが存在しない場合のみ）

   特別な指示がない場合、以下の選択肢を提示：
   ```markdown
   **質問**: タスクブランチ `feat/monorepo/20251104-turborepo-setup` をどのブランチから作成しますか？
...
1. **ユーザー入力の解析**
   ...
   - 実行フェーズ指定を自然言語から抽出（あれば）

2. **不足情報の確認**
   - 最大タスク番号が未指定の場合 → AskUserQuestionで確認
   - 実行フェーズが未指定の場合 → タスクファイルを分析して動的に質問生成
...
3. **タスクの実行を開始**
   ...
   - 実行試行IDを取得
   - **重要**: tasks.mdは更新しない（エージェントが完了時に自動更新する）
...
### 使用ツール
- **タスク読み取り**: Read、Grep、Globツール（tasks.mdの読み取りと解析用）
- **Vibe-Kanban操作**:
  - `mcp__vibe_kanban__create_task`
  - `mcp__vibe_kanban__update_task`
  - `mcp__vibe_kanban__start_task_attempt`

**注意**: tasks.mdの書き込みは行わない（実行エージェントが完了時に自動更新）
...
- **tasks.mdの書き込みは行わない**（実行エージェントが完了時に自動更新するため）
- タスクの読み取りにはRead、Grep、Globツールを使用してください
