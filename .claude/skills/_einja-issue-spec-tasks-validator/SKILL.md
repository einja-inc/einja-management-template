...
- [ ] `実行サブエージェント` が記載されている場合、`[名前]` 形式か（複数指定時はカンマ区切り: `[名前1], [名前2]`）
- [ ] `使用Skill` が記載されている場合、`[名前]` or `[steering:名前]` 形式か（複数指定時はカンマ区切り: `[名前1], [steering:名前2]`）
- [ ] インデントが正しいか（タスクグループレベル: 2スペース、タスクレベル: 4スペース）

**検出ロジック**:
```
実行サブエージェント / 使用Skill の値部分が以下の正規表現にマッチするか:
  単一: \[[\w-]+\] または \[steering:[\w.-]+\]（使用Skillのみ）
...
| `invalid_optional_metadata_format` | 任意メタデータ（実行サブエージェント / 使用Skill）の形式が不正 |
