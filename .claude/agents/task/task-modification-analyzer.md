...
2. **既存ドキュメントとの整合性確認**: requirements.md、design.md、tasks.mdの確認
...
以下の情報を受け取ります：
- ユーザーからの追加修正指示
- タスクファイルパス（例: `docs/specs/tasks/feature-name/YYYYMMDD-task-name/tasks.md`）
- 完了済みタスクID（例: `1.1.3`）
...
#### 1.1 タスクディレクトリの特定
```bash
# タスクファイルパスからディレクトリを特定
# 例: docs/specs/tasks/feature-name/YYYYMMDD-task-name/
```
...
### ドキュメントが見つからない場合
- requirements.md、design.md、tasks.mdが見つからない場合、エラーを報告
- タスクディレクトリが存在しない場合、エラーを報告
