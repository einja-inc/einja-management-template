...
design-engineerからの完了報告に「デザイン要点サマリ」が含まれている場合に実施する。

**実施条件**:
- design-engineerが実行した場合: 完了報告内の「デザイン要点サマリ」セクションを使用
- task-executerが実行した場合（デザイン要点サマリなし）: 「デザイン忠実性チェック: SKIP（design-engineer未使用）」と記録し、**警告として出力**

**チェック方法**（Pencil MCP不要 — テキストベース照合）:
1. デザイン要点サマリの「主要プロパティ」テーブルを読み取る
2. 実装コードの対応する値（Tailwindクラス、CSS値）と照合する
3. 照合対象:
   - レイアウト構造（flex方向、grid配置）
   - カラー値（背景色、テキスト色、ボーダー色）
   - スペーシング（gap、padding、margin）
   - opacity
   - インタラクション（hover、transition等）

**判定基準**:
- **PASS**: デザイン要点サマリの全項目が「一致 ✅」
- **MINOR**: 軽微な差異（1-2項目の微差、例: gap-3 vs gap-4）
- **MAJOR**: 重大な乖離（レイアウト方向の不一致、カラーの大幅な差異、opacity未設定等）

### 3. プロジェクト固有ガイドラインの最終確認

`docs/einja/steering/development/review-guidelines.md` を読み込み、einja-review-codeの8観点でカバーされない**プロジェクト固有のガイドライン違反**がないか最終確認する。
...
- 相対パスの使用がある
- デザイン要点サマリとの重大な乖離（レイアウト・カラー・opacity等）
...
- **デザイン忠実性**: [✅ 合格 / ⚠️ 軽微な差異 / ❌ 重大な乖離 / ⏭️ SKIP（design-engineer未使用）]
