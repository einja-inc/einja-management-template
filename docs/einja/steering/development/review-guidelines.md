...
react-doctor（`npx -y react-doctor@latest`）が検出する主要パターン。レビュー時の参考情報として活用：

- [ ] **useEffect内でのデータフェッチ**: useEffectでfetchを直接呼び出していないか
  - 推奨: Tanstack Query等のデータフェッチライブラリを使用
- [ ] **派生状態のuseEffect管理**: 計算で導出可能な値をuseEffect + useStateで管理していないか
  - 推奨: useMemoまたは直接計算で導出
- [ ] **コンポーネント内でのコンポーネント定義**: レンダー関数内でコンポーネントを定義していないか
  - 推奨: コンポーネントをファイルトップレベルまたは別ファイルに移動
- [ ] **バレルインポート（index.ts再エクスポート）**: バレルファイル経由のインポートでバンドルサイズが肥大化していないか
  - 推奨: 直接ファイルパスでインポート（プロジェクトの既存ルールと一致）
- [ ] **不要な再レンダリング**: メモ化されていないコールバックやオブジェクトがpropsとして渡されていないか
  - 推奨: useCallback/useMemoの適切な使用
- [ ] **アクセシビリティ**: img要素のalt属性、フォーム要素のlabel等が欠落していないか
