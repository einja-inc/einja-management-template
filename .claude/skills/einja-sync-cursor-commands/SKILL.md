---
name: einja-sync-cursor-commands
description: ".claude/skills配下のSkillを.cursor/rules用に自動変換（Cursor 2.2+対応）"
user-invocable: true
---
...
あなたは Claude Code の Skill を **Cursor 2.2+ 互換のルール形式**に自動変換する専門家です。

`.claude/skills/` 配下の全 SKILL.md ファイルをスキャンし、Cursor の**新しいルール形式**（`.cursor/rules/{rule-name}/RULE.md`）に変換します。
...
**実行手順**:
```bash
# .claude/skills 配下の全 SKILL.md ファイルを再帰的に検索
...
**ルール名の抽出**:
ディレクトリ名からルール名を抽出します（例: `einja-issue-exec/SKILL.md` → `einja-issue-exec`）。

**除外対象**:
以下のファイルは変換対象から除外します：
- `einja-sync-cursor-commands/SKILL.md`（このSkill自体。Claude Code専用機能のため）
...
- `description`: 元のSkillの description をそのまま使用
- `globs`: 空のまま（手動呼び出し用 = `@{skill-name}` で呼び出し）
...
変換後のファイルを `.cursor/rules/{skill-name}/RULE.md` に Write します：

```bash
# 例（フォルダ形式）
.claude/skills/einja-task-exec/SKILL.md → .cursor/rules/einja-task-exec/RULE.md
.claude/skills/einja-issue-spec-create/SKILL.md → .cursor/rules/einja-issue-spec-create/RULE.md
.claude/skills/einja-start-dev/SKILL.md → .cursor/rules/einja-start-dev/RULE.md
```
...
| ルール名 | 出力先 | サブエージェント参照 |
|---------|--------|-------------------|
| einja-issue-spec-create | `.cursor/rules/einja-issue-spec-create/RULE.md` | 3箇所変換 |
| einja-task-exec | `.cursor/rules/einja-task-exec/RULE.md` | 4箇所変換 |
| einja-start-dev | `.cursor/rules/einja-start-dev/RULE.md` | なし |

### 除外されたファイル
- ⏭️ einja-sync-cursor-commands/SKILL.md（Claude Code専用機能のため除外）

### 使い方
Cursor で以下のルールが利用可能になりました：
- チャットで `@einja-issue-spec-create` と入力して呼び出し
- チャットで `@einja-task-exec` と入力して呼び出し
- チャットで `@einja-start-dev` と入力して呼び出し
...
変換結果:
├── .cursor/rules/einja-issue-spec-create/RULE.md (3箇所のサブエージェント参照を変換)
├── .cursor/rules/einja-task-exec/RULE.md (4箇所のサブエージェント参照を変換)
├── .cursor/rules/einja-start-dev/RULE.md (変換不要)
└── .cursor/rules/einja-update-docs-by-issue-specs/RULE.md (変換不要)
```
