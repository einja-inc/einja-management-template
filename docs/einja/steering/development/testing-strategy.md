...
実装レイヤーだけでなく観点ごとにテスト対象を洗い出すと、抜け漏れが減る。新規機能では、少なくとも以下の観点を確認する。

| 観点 | 主に見る内容 | 推奨レベル |
|------|-------------|-----------|
| 状態パターン | 読み込み中 / 通常表示 / 保存中 / 成功 / 失敗 | Browser |
| 入力ルール | 必須、形式、範囲、相関チェック | Unit, Browser |
| 権限制御 | 閲覧、編集、API実行可否 | Integration, Browser |
| 画面遷移 | 保存後、キャンセル、離脱確認 | Browser |
| 処理フロー | 保存成功 / 保存失敗 / 再試行 | Integration, Browser |
| 表示・計算 | 読み取り専用表示、警告表示、導出値 | Unit, Browser |
| API契約 | HTTPステータス、エラーコード、レスポンス形 | Integration |
...
- `GET` 初期表示APIが必要データを返す
- `PATCH / POST` がバリデーション違反を正しく拒否する
- 権限なしユーザーのAPI呼び出しを拒否する
- 保存失敗時に再試行可能な契約を返す
...
// 注: posts テーブルは本リポジトリには未実装の仮想例（テスト戦略の説明用）
import { rpc } from '@/lib/api/rpc'
import { db } from '@repo/server-core/db/client'
import { users } from '@repo/server-core/db/schema'
// import { posts } from '@repo/server-core/db/schema' // 未実装テーブルの例

describe('Post API統合テスト', () => {
  beforeEach(async () => {
    // テストデータのクリーンアップ
    // await db.delete(posts) // 未実装テーブルの例
    await db.delete(users)
  })
...
- 「保存中はボタンが非活性化され、多重送信が防止される」
- 「権限なしロールでは編集ボタンが表示されない」
- 「不正入力時にエラーメッセージが表示され保存が阻止される」
- 「保存失敗後に入力内容が保持され再試行できる」
...
上記だけでは不十分で、状態差分やユーザー価値まで検証する必要がある。
  ...
  test('クリーンアップジョブが実行されると、期限切れセッションが削除される', async () => {
    ...
    const [expiredSession] = await db
      ...
      .values({
        ...
        userId: 'user1',
        expires: new Date(Date.now() - 1000 * 60 * 60), // 1時間前
      })
      .returning()
    ...
    const [deletedSession] = await db
      .select()
      .from(sessions)
      .where(eq(sessions.sessionToken, expiredSession.sessionToken))
      .limit(1)
...
import { db } from '@repo/server-core/db/client'
import { sessions, accounts, users } from '@repo/server-core/db/schema'

// 各テスト前にデータベースをクリーンアップ
// 外部キー制約を考慮した順序で削除する
// 注: posts テーブルは本リポジトリには未実装
beforeEach(async () => {
