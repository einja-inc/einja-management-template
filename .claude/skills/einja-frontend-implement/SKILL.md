...
AskUserQuestion:
  ...
  options:
    ...
    - label: "Pencil（.pen）ファイルを参照"
      description: |
        design-engineerエージェントがPencil MCPで.penファイルからデザイントークンを抽出します。
        メリット: Pencil.devで作成したUIモックアップを直接参照でき、デザインと実装の乖離を最小化。.penファイルはgit管理可能。
        デメリット: Pencil.devが起動している必要があり、Figma MCPほどエコシステムが成熟していません。
```
...
**プロンプト例（Pencil .penファイル）**:
```
機能名: {$ARGUMENTS}
Pencilモード: 指定.penファイルからデザイントークンを抽出

以下を実行してください：
1. Pencil MCPで.penファイルを分析
2. カラー、タイポグラフィ、スペーシングをTailwind CSSトークンに変換
3. コンポーネントバリアントをcvaレシピとして生成
4. レスポンシブブレークポイントを定義

以下のファイルを生成：
- globals.css への追加CSS変数
- components/ui/{component-name}/variants.ts
```
