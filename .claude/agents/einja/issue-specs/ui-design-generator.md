...
### hi-fi ステップ3b: design-component-manifest.json の生成

ui-design.pen の全フレームを走査し、以下の情報を収集して `design-component-manifest.json` を出力する:

1. `_components/` または `reusable: true` のフレームを収集（共有コンポーネント候補）
2. 対象アプリ（web → `@repo/ui`、admin → `@repo/admin-ui`）の package.json exports を確認
3. 突き合わせ結果として以下を生成:

```json
{
  "generatedAt": "ISO8601",
  "sourceFile": "ui-design.pen",
  "targetPackage": "@repo/ui | @repo/admin-ui",
  "components": [
    {
      "name": "ApprovalDialog",
      "frameName": "approval-dialog",
      "reusable": true,
      "screens": ["kanban", "kanban-card-detail"],
...
保存先: `{spec_dir}/design-component-manifest.json`
