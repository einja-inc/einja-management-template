...
**重要**: Vibe-Kanban経由でタスクが実行される場合、PR作成の責任は以下のように分担されます：

| 実行経路 | PR作成責任者 | 説明 |
|----------|-------------|------|
| `task-exec`コマンド経由 | `task-finisher`エージェント | task-execコマンド内でtask-finisherが明示的にPRを作成 |
| Vibe-Kanban経由 | 実行エージェント（Vibe-Kanban起動） | タスク完了時にエージェントがPRを作成し、GitHub Issueをレビュー中状態に更新 |

**Vibe-Kanban経由でのPR作成フロー**:
1. `start_task_attempt`でエージェントが起動
2. エージェントがタスクを実装
3. 実装完了後、エージェントが`gh pr create`でPRを作成
4. GitHub Issueのステータスマークを`<!-- 👀 レビュー中 -->`に更新
5. Vibe-Kanbanタスクを完了状態に更新
...
これらはすべて、Vibe-Kanban経由で起動されたエージェントが自動的に行います。
...
1. **Vibe-Kanbanステータス確認**（新規追加）

   待機ループの各サイクルで、現在実行中のタスクのステータスを確認：

   ```javascript
   // 最後に開始したタスクのステータスを確認
   const task = await mcp__vibe_kanban__get_task({
     project_id: "<project_id>",
     task_id: "<last_started_task_id>"
   });
   ```
   ...
   **ステータスに応じた分岐**:
   ...
   | ステータス | 動作 |
   |-----------|------|
   | `in-progress` | 継続待機（タスク実行中） |
   | `in-review` | 警告メッセージ表示 → 対話的確認（4.2.1参照） |
   | `done` | 待機ループを終了し、次タスク選定へ |
   | エラー/未発見 | 警告ログを出力し、GitHub Issue確認へフォールバック |

#### 4.2.1 in-review状態での対話的確認

`in-review`ステータスを検出した場合、**AskUserQuestion**ツールで確認：

**警告メッセージ**:
```
⏳ タスクグループ {task_group_number} はレビュー待ち状態です

- **Vibe-Kanbanステータス**: in-review
- **PR番号**: #{pr_number}（取得可能な場合）
...
**質問**:
```
タスクがレビュー待ち状態です。どうしますか？
```

**選択肢**:
1. **待機を継続**（推奨）: PRがマージされるまで待機ループを継続
2. **別タスクを選定**: 依存関係のない別のタスクグループがあれば選定して続行
3. **ループを終了**: 待機ループを終了し、手動介入を待つ

**選択に応じた動作**:
- 「待機を継続」→ 通常の待機処理へ進む
- 「別タスクを選定」→ task-starterで別タスクを選定（in-reviewタスクはスキップ）
- 「ループを終了」→ ループ終了メッセージを出力して終了
...
#### 4.2.2 待機試行カウンターの確認
   ...
   - 待機試行回数が30回に達した場合:
     ...
     最後に確認したステータス:
     - Vibe-Kanban: {last_status}
     - GitHub Issue: {issue_status}

     **推奨アクション**:
     1. Vibe-Kanban UIでタスク状態を確認してください
     2. 問題がある場合は手動で対処してください
     3. 再開する場合は `/task-vibe-kanban-loop #{issue_number}` を実行してください
...
- **Vibe-Kanban操作**:
  ...
  - `mcp__vibe_kanban__get_task`（ステータス確認用）
  - `mcp__vibe_kanban__list_tasks`（タスク一覧取得用）
