...
allowed-tools: Read, Write, Edit, Bash, Grep, Glob, mcp__github__*, mcp__serena__*, mcp__vibe_kanban__*, AskUserQuestion
---
...
- GitHub Issueから着手可能なタスクグループを選定（haiku モデル推奨）
- 選定されるタスク数は自然言語指示から自動判断
- 粒度指定（「1-1, 1-2の粒度でタスク作成」など）がある場合、各中カテゴリ（サブフェーズ）ごとに実行可能な全タスクを選定
- 明示的なタスク指定がある場合、指定された数だけ選定
- 指定がない場合、最も優先度の高い1タスクのみ選定
...
1. **Vibe-KanbanにサブIssueを作成**（task-starterが選定したタスクのみ）
   ...
   **選定タスクが1個の場合**:
   ...
   - 説明:
     ...
     ## タスク一覧
     <tasks.mdから抽出したタスク一覧（X.Y.Z形式）とサブタスク>
     ```
...
→ 各タスクを1個ずつ選定して、該当フェーズブランチで実行（デフォルト動作）
