...
4. **task-finisher**: PR作成・レビュー待ちフェーズ
   - PRを作成（`gh pr create`コマンド使用）
   - GitHub Issueのステータスを「レビュー中」に変更
   - 着手中マーク → レビュー中マーク（`<!-- 👀 レビュー中 -->`）に置換
   - **⚠️ 重要**: 完了マーク（`[x]`）は設定しない - 人間がPRマージ後に手動で設定
...
#### 5. PR作成・レビュー待ちフェーズ（task-finisher）
- task-qaサブエージェント完了後に開始（全テスト合格が前提）
- task-finisherサブエージェントを呼び出し
- **PR作成**:
  - `gh pr create`コマンドでIssueブランチへのPRを作成
  - PRタイトル: `[{タスクグループ番号}] {タスクグループ名}`
- **GitHub Issue更新**:
  - 着手中マーク（`<!-- 🔄 着手中 -->`）をレビュー中マーク（`<!-- 👀 レビュー中 -->`）に置換
  - **⚠️ 重要**: 完了マーク（`[x]`）は設定しない - 人間がPRマージ後に手動で設定
- **完了時**: task-finisherが生成したレビュー待ち報告を表示し、コマンドを終了
