...
- **description**: いつトリガーするか、何をするか。主要なトリガーメカニズム。Skillが何をするかと使用する具体的なコンテキストの両方を含める。「いつ使用するか」情報はすべてここに。本文はトリガー後に読み込まれるため、本文の「使用すべき場合」セクションはClaudeに役立たない。注意：現在Claudeはスキルを「アンダートリガー」する傾向がある。対策としてdescriptionを少し「積極的」にする
  ...
  **ネガティブトリガー**: 類似Skillとの混同を防ぐため、除外条件を明示する:
  ```
  Do NOT use for: [除外シーン]（→ 代替skill名）
  ```

  **ファイルタイプ言及**: 特定のファイルタイプを処理するSkillでは、対象拡張子をdescriptionに含める（例: `.pdf`, `.docx`, `.csv`）

  **良い例**:
  ```
  description: PDFファイルの読み取り・作成・編集・変換を行う。.pdfファイルの操作、フォーム入力、ページ分割・結合、テキスト抽出が必要な場合に使用。Do NOT use for: 画像ファイルの処理（→ image-processor）
  ```
  ...
  **悪い例**:
  ...
  ```
  → 何のドキュメントか不明、トリガー条件が曖昧、除外条件なし
- **compatibility**: 必要なツール、依存関係（オプション、まれに必要）
- **metadata**（オプション）: author、version、mcp-server等のメタ情報。例:
  ```yaml
  metadata:
    author: team-name
    version: 1.0.0
...
テストケースは以下の3エリアをカバーすること:

1. **Triggering（トリガー）**: Skillが適切に呼び出されるか
   - 正しいクエリでトリガーされる（true positive）
   - 無関係なクエリではトリガーされない（true negative）
2. **Functional（機能）**: Skillが期待通りの出力を生成するか
   - Given: [前提条件] → When: [操作] → Then: [期待結果]
3. **Performance comparison（パフォーマンス比較）**: Skillなしと比較して改善されているか
...
Skillの品質を測定するための基準を設定する。

### 定量的基準
- **トリガー率**: 意図したクエリで90%以上トリガーされることを目標
- **ツール呼び出し効率**: Skillなしと比較してツール呼び出し数が削減されているか
- **エラー率**: API呼び出し失敗・スクリプト実行エラーが0件

### 定性的基準
- **自律完了**: ユーザーの追加介入なしにタスクを完了できるか
- **修正不要**: 出力に手動修正が不要か
- **セッション間一貫性**: 異なるセッションで同じプロンプトに対して一貫した品質の結果を返すか

成功基準はSkillの種類に応じて調整する。主観的なSkill（文体、デザイン）は定性的基準を重視し、ツール操作系Skillは定量的基準を重視する。
...
- `references/checklist.md` — Skill開発の4フェーズチェックリスト
- `references/design-patterns.md` — ユースケースカテゴリとデザインパターン
- `references/troubleshooting.md` — トリガー問題・指示遵守問題の対処法
